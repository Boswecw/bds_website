...
**Status:** Phases 1–3 implemented (enrollment, sign-in challenge, session gating, backup-authenticator recovery, ForgeCustomer-side AAL2 enforcement). Phase 4b implemented (ForgeCustomer admin endpoint, Forge_Command "Require MFA" operator action, bds_website error-handling fix) — see §10. Phase 4a remains not started, blocked on Sentinel infrastructure that doesn't exist yet. Phase 5 implemented (MFA made mandatory for all accounts, with a 30-day grace period and an in-app reminder banner) — see §13.
...
- ~~Making MFA mandatory for all accounts — ship opt-in first; mandating it... is a product decision, not an engineering default.~~ **Resolved and implemented** — see §13. Mandatory for all accounts (not scoped to subscribers/operators), via a grace period rather than immediate enforcement.
...
5. **Mandatory MFA rollout** (§13). **✅ Implemented** (`forgecustomer` PR #20, `bds_website` PR #22). MFA is now required on every account, enforced via a 30-day grace period rather than immediately, with an in-app reminder banner.

## 13. Phase 5 — Mandatory MFA rollout (all accounts, grace period)

§4 and §10 both flagged the same open question: should MFA ever stop being opt-in? That's now decided and implemented, resolving the two open questions this section replaces.

**The decision.** MFA becomes mandatory for **every** account — not scoped to active subscribers, stored-payment-method holders, or Forge_Command operator accounts specifically, the narrower options considered in §4's original framing. Existing accounts that haven't enrolled are not locked out immediately: they get a 30-day grace period during which the account behaves as it always has (AAL1 accepted), plus a reminder to enroll. Once the grace period elapses without an enrolled factor, enforcement becomes exactly what §9 already shipped — an immediate AAL2 requirement on every `CustomerContext`-gated request.

A grace period is necessary here for a reason self-service and operator-forced enforcement never had to consider: those two paths only ever flip `mfa_required` to `true` for an account that's already aal2 (self-service) or as a deliberate, immediate incident-response action (operator-forced) — see below. Flipping it for the *entire* existing customer base at once, most of whom have never been asked to enroll, is different in kind: instant enforcement would lock out every customer who hadn't already opted in, the moment the migration ran.

**Mechanics (ForgeCustomer, PR [#20](https://github.com/Boswell-Digital-Solutions/forgecustomer/pull/20)):**

- New column `customer_profiles.mfa_grace_period_ends_at timestamptz`, nullable.
- Migration `0014_mandatory_mfa_grace_period.sql`: for every existing row where `mfa_required` was `false`, sets `mfa_required = true` and `mfa_grace_period_ends_at = now() + interval '30 days'`. Then changes the column defaults going forward — `mfa_required` now defaults to `true`, and `mfa_grace_period_ends_at` now defaults to `now() + interval '30 days'` — so every newly-provisioned account is subject to the same policy and the same window from day one, with no application-code change needed to keep applying it.
- `CustomerContext` gained `mfa_grace_active()` (`self.mfa_grace_period_ends_at.is_some_and(|deadline| deadline > Utc::now())`). `require_active()` changed from unconditionally calling `require_aal2()` when `mfa_required` is set, to skipping that call while `mfa_grace_active()` is true. Once the deadline passes — or for any account that never had a grace period recorded at all — behavior is identical to before this phase.
- `GET /v1/account` now echoes `mfa_grace_period_ends_at` alongside the existing `mfa_required`, so the client can render a reminder without a new endpoint.
- Deliberately untouched: the self-service (§9, `POST /v1/account/mfa-status`) and operator-forced (§10, 4b's admin endpoint) paths never set or read `mfa_grace_period_ends_at` — both continue to enforce the moment they flip the flag. Self-service can only be invoked by a caller who already holds an aal2 token, meaning they've already completed a real TOTP challenge — there is no one left to grace-period. Operator-forced is deliberate, human-initiated incident response; the entire point of that action is immediate effect, and a grace period would silently defeat it.

**Mechanics (bds_website, PR [#22](https://github.com/Boswell-Digital-Solutions/bds_website/pull/22)):** `bootstrapSession()` (`session.js`) now calls `showMfaGraceReminder()` once provisioning succeeds. It reads `mfa_grace_period_ends_at` from `GET /v1/account` (cached in `sessionStorage` for the tab's lifetime — it doesn't change during a session), and while it's in the future *and* the account currently has zero enrolled factors (checked live via `listFactors()` on every call, never cached), renders a persistent banner at the top of the page: "Two-factor authentication is now required on every account. You have N days left to set it up," linking to `/account.html`. Because `bootstrapSession()` is the shared chokepoint every authenticated page already calls, this needed no page-by-page wiring — it surfaces everywhere someone can already reach while signed in, and disappears on the very next page load after they enroll.

**Reminders — scope decision: in-app banner only, for now.** Email reminders were explicitly considered and deferred, not silently dropped. Neither this repo nor ForgeCustomer has any email-sending infrastructure today — confirmed against ForgeCustomer's own `doc/system/30_dependencies/06-integrations-events.md`, which lists only Supabase, Stripe, and DataForge as integrations, with DataForge explicitly documented as forbidden from carrying PII or email addresses. Standing up email (provider choice, deliverability, unsubscribe handling, PII storage) is a real infrastructure and vendor decision, bigger than this feature and outside this plan's scope — it's a follow-on if the in-app banner proves insufficient, not a gap in this phase.

**What happens at grace-period expiry, traced through the code:** once `mfa_grace_period_ends_at` is in the past and the account still has zero factors, `mfa_grace_active()` returns `false`, so `require_active()` enforces AAL2 immediately on every `CustomerContext`-gated route — including `GET /v1/account` itself, which starts returning `403 MFA_REQUIRED`. That's the exact same response §10's 4b work already taught `errors.js` to handle (inline "go set it up" message; no sign-out; no redirect loop). `showMfaGraceReminder()`'s own `forge.account()` call fails the same way, but it's wrapped in a try/catch that just skips the banner for that page load — so there's no double messaging and no crash, and the pre-existing `MFA_REQUIRED` handling is what the customer actually sees. Verified live in the bds_website Playwright suite (grace-banner scenario D).

## Open questions

- Is project-level MFA already enabled in the Supabase Auth dashboard for this project, or does that need to be turned on first? Still unverified. Checked via the Supabase MCP connector against the real project (`forgecustomer`, ref `sffgcltrwewnjzvrzjak` — confirmed the right one by its table set) — but the connector's tools (`list_tables`, `execute_sql`, `get_advisors`, migrations/branches/edge-functions) are database- and project-management-focused and don't reach Auth's provider configuration (Authentication → Sign In / Providers → Multi-Factor Authentication), which lives outside Postgres. The security advisor's silence on MFA isn't evidence either way — its checks (RLS gaps, `SECURITY DEFINER` exposure, leaked-password-protection) don't include an MFA-enabled/disabled lint at all. Still needs an actual look at that dashboard page.
- **New, found while checking the above:** the Supabase security advisor flags **leaked password protection** (checks new passwords against HaveIBeenPwned) as disabled for the `forgecustomer` project — a real, unrelated gap in the same account-security area as this whole plan. Cheap to turn on (a toggle in Authentication → Policies, no code change), not scheduled work here since it's outside this plan's TOTP-MFA scope, but worth doing. See https://supabase.com/docs/guides/auth/password-security#password-strength-and-leaked-password-protection
- ~~Should MFA ever be mandatory (all accounts, or just accounts with an active subscription / stored payment method / Forge_Command operator role)?~~ **Resolved and implemented** (`forgecustomer` PR #20, `bds_website` PR #22) — see §13. Mandatory for all accounts, via a 30-day grace period rather than immediate enforcement, with an in-app-only reminder for now (email reminders explicitly deferred, pending real email infrastructure in either repo).
- ~~Confirm the exact `auth.mfa` method shapes against the pinned `@supabase/supabase-js@2.45.0` release before relying on them further...~~ **Resolved.** That version resolves `@supabase/auth-js@2.64.4`; its published type definitions match what this repo's `mfa.js` already assumed, exactly: `enroll()` returns `{id, totp: {qr_code, secret, uri}}`; `challenge()` returns `{id, expires_at}`; `verify({factorId, challengeId, code})` and `unenroll({factorId})` take exactly the fields passed; `listFactors()` returns `{all, totp}` with each `Factor.status` typed as the literal union `'verified' | 'unverified'`; and — the one that actually matters for security — `getAuthenticatorAssuranceLevel()` returns `currentLevel`/`nextLevel` typed as the literal union `'aal1' | 'aal2'`, so `hasPendingChallenge()`'s `data.nextLevel === "aal2"` string comparison is exactly right, not a lucky guess. No code changes; this was a verification, not a bug.
...
7. ~~Decide and, if approved, implement whether MFA should ever become mandatory (§4/§10's open question).~~ Done — mandatory for all accounts, via a 30-day grace period, in-app banner only for now (§13): ForgeCustomer's grace-period schema and enforcement (`forgecustomer` PR #20) and bds_website's site-wide reminder banner (`bds_website` PR #22).
